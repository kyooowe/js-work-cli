...
async function compileArguments () {
    ...
    if (framework === 'react') { await reactWorker(__dirname, template, path, fileName, fileType) } else if (framework === 'express') { await expressWorker(__dirname, template, path, fileName, fileType) } else { ErrorMessage() }
}
