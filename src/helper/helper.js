...
const DependenciesChecker = (dArr, dRequiredArr, fileType, dWithTypes) => {
    let rDependencies = []
    let scripts = ''
    let toBeInstalledDependencies = []
    let isTypescriptInstalled = true

    // Map through given required dependencies
    dRequiredArr.map((d) => {

        let dependency = { name: d }
        const isDependencyInstalled = dArr.find((s) => s === d)

        if (isDependencyInstalled === undefined)
            dependency.isAlreadyInstalled = false
        else
            dependency.isAlreadyInstalled = true

        rDependencies.push(dependency)
    })

    // Variable for missing dependencies
    let missingDependencies = rDependencies.filter((d) => !d.isAlreadyInstalled)
    ...
    if (fileType !== 'ts') {
        ...
        missingDependencies.map((d) => {
            ...
            if (isDependencyNeedTypes === undefined)
                scripts = `${scripts} ${d.name}`
            else
                scripts = `${scripts} ${d.name} @types/${d.name}`
